...
dff = "dff$ {dff_name}({clk}, {d}, {q}, {qbar}, {r}, {s});"
@dataclass
class GATES:
    gate_type: str
    n_gates: int
...
def cube_to_gate(pla):
    ...
    state_wire_list = []
    state_code_list = []
    for i in range(pla.n_states):
        state_name = pla.input_names[i]
        next_state_name = pla.output_names[i]
        state_inv_name = f"{state_name}_inv"
        state_wire_list.append(state_name)
        state_wire_list.append(state_inv_name)
        dff_inst_name = f"dffinst{i}"
        if pla.init_state[i] == '1':
            state_code = dff.format(
                dff_name=dff_inst_name,
                clk="clk",
                d=next_state_name,
                q=state_name,
                qbar=state_inv_name,
    ...
    if len(state_wire_list) > 0:
        wire_code_list.append(get_wire_code(state_wire_list))

    for inp_pat, out_pat in pla.cubes:

        for i in range(pla.n_states, pla.n_inputs):
                if inp_pat[i] == "0":
    ...
    wire_code_list.extend(state_code_list)
